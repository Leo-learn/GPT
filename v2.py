...
class BigramLanguageModel(nn.Module): # Bigram subclass of the nn.module
  ...
  def __init__(self):
    ...
    self.token_embedding_table = nn.Embedding(vocab_size, num_embd_dim)
    # nn.Embedding is just a wrapper
    # we dont only want to encode the identity of the token but also want to store there position
    self.position_embedding_table = nn.Embedding(block_size, num_embd_dim)
    # going from tokens to logits we will need a linear layer
    self.lm_head = nn.Linear(num_embd_dim, vocab_size)
